...
impl CardRenderer {
    pub fn new(position: Vec2, cost: u16, attack: u16, defense: u16, name: String) -> Self {
        Self {
            position,
            width: 180.0,
            height: 220.0,
    ...
    pub fn draw_card(&self) {
        ...
        let name_x = self.position.x + self.width / 2.0
        ...
        draw_text(
            ...
            self.position.x + 10.0,
            self.position.y + self.height - 20.0,
            20.0,
            GREEN,
        );
        ...
        let def_x =
            self.position.x + self.width - measure_text(&defense_text, None, 20, 1.0).width - 10.0;
        draw_text(
            &defense_text,
            def_x,
            self.position.y + self.height - 20.0,
